...
## 🥕 왜 동시성 프로그래밍이 필요할까?
1. 사용성, 반응성을 제공하기 위해  
예를 들어 인스타그램이 한개의 쓰레드로 동작한다면 사진을 스크롤하며 로딩하는 시간이 엄청나게 길어질 것. 
...
+ 동기/비동기 : 메인 스레드(?)에서 작업이 끝나길 기다리냐? 안기다리냐? 
+ 직렬/동시성 : 단일 스레드로 하냐? 다중 스레드로 하냐?
...
+ 동시성 VS 병렬성 :   
반대개념은 아니다  
동시성은 다중코어, 동시성은 멀티 스레드를 활용하는 것  
예) 라떼주문   
동시성 : 직원1(에쏘내리면서, 스팀치고 컵준비해서 완성)  
병렬성 : 직원1(컵준비), 직원2(에쏘내리기), 직원3(스팀치기)  
