...
Controller 대신에 Presenter, View와 Model의 의존성을 없애기 위해. 

애플의 MVC랑 비슷하다. 하지만 같지는 않다. MVP의 Presenter는 ViewController의 라이프사이클의 영향을 미치지 않는다. View는 Moked를 만들수 있음. Presenter가 레이아웃 관련코드 없이 View와 분리가 되어

장점 : 
* 개별적으로 분리 되어 있기 때문에 각 요소를 독립적으로 테스트 가능

단점 :
* View 와 Presenter 가 1 대 1 관계를 가진다. 코드가 길어짐. 
* 
